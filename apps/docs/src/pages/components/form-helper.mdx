  ...
  const { formComponent, form } = useFormHelper(
    {
      ...
      fields: [
        ...
        { name: 'date', type: 'date', label: 'Date', size: 'half', dateInputProps: {/* Props for DatePicker */} },
        {
          ...
          label: 'Role',
          options: [
            { value: UserRole.ADMIN, label: 'Trainer' },
            { value: UserRole.MEMBER, label: 'Super Participant' },
          ],
          size: 'full',
        },
...
- `select`: Will render a select dropdown.
  - Preferred zod schema is a `z.nativeEnum` or `z.enum`. The
    ...
    `[ { value: UserRole.ADMIN, label: 'Trainer' }, { value: UserRole.MEMBER, label: 'Super Participant' }, ]`,
